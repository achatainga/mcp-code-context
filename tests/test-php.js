...
async function main() {
    ...
    if (fs.existsSync(configPath)) {
        ...
        log(`Dt24Config.php: ${originalLines} → ${compressedLines} lines (${ratio}% reduction)`, compressed);
    ...
    if (fs.existsSync(adminPath)) {
        ...
        const compressedLines = compressed.split("\n").length;
        const ratio = ((1 - compressedLines / originalLines) * 100).toFixed(1);
        log(`Admin: ${originalLines} → ${compressedLines} lines (${ratio}% reduction)`, compressed);
        const checks = [
...
`;
    ...
    const compressedLines = compressed.split("\n").length;
    const ratio = ((1 - compressedLines / originalLines) * 100).toFixed(1);
    log(`Synthetic: ${originalLines} → ${compressedLines} lines (${ratio}% reduction)`, compressed);
...
//# sourceMappingURL=test-php.js.map