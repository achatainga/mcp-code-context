...
async function runTests() {
  ...
  try {
    ...
    if (backupsAfterLimit.length !== 5) throw new Error(`Expected 5 backups, got ${backupsAfterLimit.length}`);
    ...
    if (!rb1.success) throw new Error("Rollback failed: " + rb1.error);
    
    const content1 = await fs.readFile(testFile, "utf-8");
    if (content1 !== "v7") throw new Error(`Expected v7, got ${content1}`);
    ...
    if (content2 !== "v6") throw new Error(`Expected v6, got ${content2}`);
    ...
    if (cleanResult.deletedCount !== 5) throw new Error(`Expected 5 deleted, got ${cleanResult.deletedCount}`);
