...
function printNode(node, depth = 0) {
    ...
    if (node.kind) {
        const info = [`${indent}[${node.kind}]`];
        if (node.name) {
            const name = typeof node.name === "string" ? node.name : node.name?.name || "";
            info.push(`name="${name}"`);
        }
        if (node.visibility)
            info.push(`visibility="${node.visibility}"`);
        if (node.isStatic)
            info.push("static");
        if (node.isAbstract)
            info.push("abstract");
        ...
        if (node.implements && node.implements.length > 0) {
            info.push(`implements=[${node.implements.map((i) => i.name || i).join(", ")}]`);
        }
...
printNode(ast);
//# sourceMappingURL=test-php-ast.js.map