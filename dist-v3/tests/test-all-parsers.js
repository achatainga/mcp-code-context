...
async function test() {
    ...
    if (!tsParser)
        throw new Error("TS parser not found");
    const tsCode = `
...
`;
    ...
    if (!phpParser)
        throw new Error("PHP parser not found");
    const phpCode = `<?php
function hello($name) {
    return "Hello, " . $name;
}
...
?>`;
    ...
    if (!extracted)
        throw new Error("Failed to extract 'add'");
    ...
    if (!newCode.includes("function add"))
...
//# sourceMappingURL=test-all-parsers.js.map