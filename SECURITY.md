# Security Policy - mcp-code-context v3.5.1

## 🔒 Security Improvements in v3.5.1
...
**CVSS Score**: 8.1 (High)
**Status**: ✅ FIXED in v3.2.0, **hardened in v3.5.1**

**Description**: Path traversal check occurred BEFORE path normalization, allowing bypass with absolute paths.

**Fix**: Check boundary AFTER `path.resolve()` normalization. In v3.5.1, **all 13 handlers** now require `projectRoot` and validate via `SecurityValidator`.
