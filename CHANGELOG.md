...
### Fixed
- **npm package**: Moved `typescript` from devDependencies to dependencies
  - Reason: `safeRename.ts` uses TypeScript compiler API at runtime
  - Impact: Fixes `ERR_MODULE_NOT_FOUND` error when running via npx
